...
const FILTER_TABS: { label: string; value: AnalyticStatus | "all" }[] = [
  { label: "New", value: "new" },
...
export default function AnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<AnalyticStatus | "all">("confirm");
  const [dialog, setDialog] = useState<{ type: "create" | "edit"; row?: AnalyticAccount } | null>(null);

  const filtered = useMemo(() => (activeTab === "all" ? rows : rows.filter((r) => r.status === activeTab)), [rows, activeTab]);
      ...
      <div className="space-y-8">
        <header className="rounded-[32px] border border-brand-primary/30 bg-slate-900/95 p-6 text-brand-light shadow-[0_25px_80px_rgba(15,23,42,0.45)]">
            ...
            {FILTER_TABS.map((tab) => (
              <button
                ...
                className={`rounded-full border px-5 py-2 transition ${
                  activeTab === tab.value ? "border-brand-primary bg-brand-primary/10 text-white" : "border-brand-primary/30 text-brand-light hover:border-brand-primary/60"
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="mt-6 flex items-center justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-brand-accent">Analytics Master</p>
              <h1 className="text-3xl font-semibold text-white">Cost Centers</h1>
            </div>
          </div>
        </header>
