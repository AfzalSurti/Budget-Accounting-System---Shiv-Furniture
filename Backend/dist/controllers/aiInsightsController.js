...
export const listRiskInsights = async (companyId, start, end) => {
    const now = new Date();
    ...
    const riskItems = [
        ...overdueInvoices.map((invoice) => {
            const daysOverdue = invoice.dueDate
                ? Math.max(0, Math.ceil((now.getTime() - invoice.dueDate.getTime()) /
                    (1000 * 60 * 60 * 24)))
                : 0;
            return {
                id: invoice.id,
        ...
        ...overdueBills.map((bill) => {
            const daysOverdue = bill.dueDate
                ? Math.max(0, Math.ceil((now.getTime() - bill.dueDate.getTime()) / (1000 * 60 * 60 * 24)))
                : 0;
            return {
                id: bill.id,
                type: "vendor_bill",
...
export const listOpportunityInsights = async (companyId, start, end) => {
    ...
    const opportunities = rows
        ...
        .slice(0, 6)
        .map((row) => {
        const utilization = row.budgetedAmount === 0 ? 0 : row.actualAmount / row.budgetedAmount;
        return {
...
export const listAnomalyInsights = async (companyId, start, end) => {
    ...
    const anomalies = [
        ...invoices
            .filter((invoice) => invoiceAvg > 0 && Number(invoice.totalAmount) > invoiceAvg * 1.8)
            .map((invoice) => ({
            id: invoice.id,
            type: "invoice",
            ...
            confidence: clampConfidence(0.5 + (Number(invoice.totalAmount) / invoiceAvg - 1) * 0.25),
        })),
        ...payments
            .filter((payment) => paymentAvg > 0 && Number(payment.amount) > paymentAvg * 1.8)
            .map((payment) => ({
            id: payment.id,
            type: "payment",
            ...
            confidence: clampConfidence(0.5 + (Number(payment.amount) / paymentAvg - 1) * 0.25),
        })),
    ];
    return anomalies;
};
