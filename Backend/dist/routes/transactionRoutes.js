...
// Purchase Orders
transactionRoutes.post("/purchase-orders", validateRequest(createPurchaseOrderSchema), asyncHandler(async (req, res) => {
...
transactionRoutes.get("/po/:id/pdf", asyncHandler(async (req, res) => {
    const result = await purchaseController.getPurchaseOrderPdf(req.params.id);
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="${result.filename}"`);
    res.send(result.buffer);
}));
transactionRoutes.put("/purchase-orders/:id", validateRequest(updatePurchaseOrderSchema), asyncHandler(async (req, res) => {
    const po = await purchaseController.updatePurchaseOrder(req.params.id, req.body);
    res.json({ success: true, data: po });
}));
transactionRoutes.delete("/purchase-orders/:id", asyncHandler(async (req, res) => {
    const po = await purchaseController.deletePurchaseOrder(req.params.id);
    res.json({ success: true, data: po });
}));
// Vendor Bills
transactionRoutes.post("/vendor-bills", validateRequest(createVendorBillSchema), asyncHandler(async (req, res) => {
    const bill = await vendorBillController.createVendorBill(req.body);
    res.status(201).json({ success: true, data: bill });
}));
transactionRoutes.post("/vendor-bills/from-po/:poId", validateRequest(convertVendorBillSchema), asyncHandler(async (req, res) => {
    const bill = await vendorBillController.createVendorBillFromPO(req.params.poId, req.body.billNo, req.body.billDate);
    res.status(201).json({ success: true, data: bill });
}));
...
transactionRoutes.get("/vendor-bills/:id/pdf", asyncHandler(async (req, res) => {
    const result = await vendorBillController.getVendorBillPdf(req.params.id);
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="${result.filename}"`);
    res.send(result.buffer);
}));
transactionRoutes.put("/vendor-bills/:id", validateRequest(updateVendorBillSchema), asyncHandler(async (req, res) => {
    const bill = await vendorBillController.updateVendorBill(req.params.id, req.body);
    res.json({ success: true, data: bill });
}));
// Sales Orders
transactionRoutes.post("/sales-orders", validateRequest(createSalesOrderSchema), asyncHandler(async (req, res) => {
...
transactionRoutes.get("/so/:id/pdf", asyncHandler(async (req, res) => {
    const result = await salesController.getSalesOrderPdf(req.params.id);
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="${result.filename}"`);
    res.send(result.buffer);
}));
transactionRoutes.put("/sales-orders/:id", validateRequest(updateSalesOrderSchema), asyncHandler(async (req, res) => {
    const so = await salesController.updateSalesOrder(req.params.id, req.body);
    res.json({ success: true, data: so });
}));
transactionRoutes.delete("/sales-orders/:id", asyncHandler(async (req, res) => {
    const so = await salesController.deleteSalesOrder(req.params.id);
    res.json({ success: true, data: so });
}));
// Invoices
transactionRoutes.post("/invoices", validateRequest(createInvoiceSchema), asyncHandler(async (req, res) => {
    const invoice = await invoiceController.createInvoice(req.body);
    res.status(201).json({ success: true, data: invoice });
}));
transactionRoutes.post("/invoices/from-so/:soId", validateRequest(convertInvoiceSchema), asyncHandler(async (req, res) => {
    const invoice = await invoiceController.createInvoiceFromSO(req.params.soId, req.body.invoiceNo, req.body.invoiceDate);
    res.status(201).json({ success: true, data: invoice });
}));
...
transactionRoutes.get("/invoices/:id/pdf", asyncHandler(async (req, res) => {
